...
class ImageEmbedder(nn.Module):
    ...
    def __init__(
        ...
        self._vision_model = model.vision_model.to(device)
        self._visual_projection = model.visual_projection.to(device)

        try:
            self._device = torch.device(device)
        except:
            self._device = torch.device("cpu")
            print(f"Device {device} not found, use cpu instead.")
    
    def forward(self, image_tensor):
        with torch.no_grad():
            vision_output = self._vision_model(image_tensor)
            image_embeds = vision_output[1] ## pooler_output
            image_embeds = self._visual_projection(image_embeds) ## 768 -> 512
        norm = image_embeds.norm(p=2, dim=1, keepdim=True)
        image_embeds = image_embeds.div(norm)
        image_embeds_np = image_embeds.detach().cpu().numpy()
...
class PilpProcess(object):
    def __init__(self, size):
        plip_processor = AutoProcessor.from_pretrained("vinid/plip")
        self._processor = plip_processor.image_processor
    def __call__(self, sample):
        img = sample['input']
        img = self._processor(images=img, return_tensors="pt")["pixel_values"][0]
        return {'input': img}
...
def compute_feats(args, bags_list, i_classifier, save_path=None, magnification='single'):
    ...
    for i in range(0, num_bags):
        ...
        if magnification=='single' or magnification=='low':
            csv_file_path = glob.glob(os.path.join(bags_list[i], f'*.{args.patch_ext}'))
        elif magnification=='high':
            csv_file_path = glob.glob(os.path.join(bags_list[i], '*'+os.sep+f'*.{args.patch_ext}'))
            print()
...
def compute_tree_feats(args, bags_list, embedder_low, embedder_high, save_path=None):
    ...
    with torch.no_grad():
        for i in range(0, num_bags): 
            low_patches = glob.glob(os.path.join(bags_list[i], f'*.{args.patch_ext}'))
            print(f'low_patches: {low_patches}')
            feats_list = []
            feats_tree_list = []
            ...
            for idx, low_patch in enumerate(low_patches):
                ...
                print(f'high_folder: {high_folder}')
                high_patches = glob.glob(high_folder+os.sep+f'*.{args.patch_ext}')
                if len(high_patches) == 0:
                    pass
                else:
                    for high_patch in high_patches:
                        print(f'high_patch: {high_patch}')
...
def main():
    parser = argparse.ArgumentParser(description='Compute UBC-OCEAN features from PLIP image encoder')
    ...
    parser.add_argument('--backbone', default='plip', type=str, help='Embedder backbone [plip]')
    ...
    parser.add_argument('--dataset', default='UBC-OCEAN', type=str, help='Dataset folder name [UBC-OCEAN]')
    parser.add_argument('--patch_ext', default='png', type=str, help='Patch extension [png]')
    parser.add_argument('--json_path', default='train.json', type=str, help='Path to the tile list json file [train.json]')
    ...
    if args.backbone == 'resnet18':
        feature_extractor = models.resnet18(pretrained=pretrain, norm_layer=norm)
        num_feats = 512
    if args.backbone == 'resnet34':
        feature_extractor = models.resnet34(pretrained=pretrain, norm_layer=norm)
        num_feats = 512
    if args.backbone == 'resnet50':
        feature_extractor = models.resnet50(pretrained=pretrain, norm_layer=norm)
        num_feats = 2048
    if args.backbone == 'resnet101':
        feature_extractor = models.resnet101(pretrained=pretrain, norm_layer=norm)
        num_feats = 2048
    if args.backbone == 'plip':
        feature_extractor = ImageEmbedder(model_name="vinid/plip", device="cuda")
        num_feats = 512
    ...
    if args.magnification == 'tree' and args.weights_high != None and args.weights_low != None:
        i_classifier_h = mil.IClassifier(feature_extractor, num_feats, output_class=args.num_classes).cuda()
        i_classifier_l = mil.IClassifier(copy.deepcopy(feature_extractor), num_feats, output_class=args.num_classes).cuda()
        ...
        i_classifier = mil.IClassifier(feature_extractor, num_feats, output_class=args.num_classes).cuda()
    ...
    elif args.magnification == 'tree' and args.backbone == 'plip':
        i_classifier_h = mil.IClassifier(feature_extractor, num_feats, output_class=args.num_classes).cuda()
        i_classifier_l = mil.IClassifier(copy.deepcopy(feature_extractor), num_feats, output_class=args.num_classes).cuda()
