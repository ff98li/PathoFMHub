...
def test(test_df, milnet, criterion, args, save_path):
    milnet.eval()
    total_loss = 0
    test_labels = []
    test_predictions = []
    ...
    np.savez_compressed(
        os.path.join(save_path, f'result.npz'),
        labels=test_labels,
...
def main():
    ...
    parser.add_argument('--num_classes', default=5, type=int, help='Number of output classes [2]')
    ...
    parser.add_argument('--dataset', default='UBC-OCEAN', type=str, help='Dataset folder name')
    ...
    parser.add_argument('--split_json', type=str, default=None, help='Path to train test split json file.')
    parser.add_argument('--agg_save_dir', type=str, default='baseline', help='Path to save the aggregator.')
    parser.add_argument('--load_path', type=str, default=None, help='Path to pretrained the aggregator.')
    ...
    if args.c_path:
        save_path = os.path.join(
            'deconf' + '_' + args.agg_save_dir,
            datetime.date.today().strftime("%m%d%Y"),
            str(args.dataset)+'_'+str(args.model)+'_'+str(args.agg )+'_c_path'
        )
    else:
        save_path = os.path.join(args.agg_save_dir, datetime.date.today().strftime("%m%d%Y"), str(args.dataset)+'_'+str(args.model)+'_'+str(args.agg )+'_fulltune')
    ...
    elif args.dataset.startswith('UBC-OCEAN'):
        bags_csv = os.path.join('datasets', args.dataset, args.dataset+'.csv')
        bags_path = pd.read_csv(bags_csv)
        if args.split_json:
            print(f'Using train/val split from: {args.split_json}')
            assert os.path.exists(args.split_json)
            with open(args.split_json, "r") as f:
                split_dict = json.load(f)
            train_id = split_dict["train"]["image"]
            val_id = split_dict["test"]["image"]
            temp_df = bags_path.copy()
            temp_df["id"] = temp_df["0"].apply(lambda x: os.path.basename(x).split(".csv")[0])
            train_path = temp_df[temp_df["id"].isin(train_id)]
            test_path = temp_df[temp_df["id"].isin(val_id)]
            train_path = train_path.drop(columns=["id"])
            test_path = test_path.drop(columns=["id"])
            print(f'Train size: {len(train_path)}, Test size: {len(test_path)}')
        else:
            all_examples = bags_path['0'].apply(
                lambda x: os.path.basename(x).split(".csv")[0]
            ).values
            train_examples = [ex for ex in all_examples if ex.startswith("UBC")]
            test_examples = list(set(all_examples) - set(train_examples))
            train_path = bags_path.loc[bags_path["0"].apply(
                lambda x: os.path.basename(x).split(".csv")[0] in train_examples
            )]
            test_path = bags_path.loc[bags_path["0"].apply(
                lambda x: os.path.basename(x).split(".csv")[0] in test_examples
            )]
            print(f'Train size: {len(train_path)}, Test size: {len(test_path)}')
    ...
    if args.test:
        ...
        msg = milnet.load_state_dict(state_dict_weights, strict=True)
        print('Missing these:', msg.missing_keys)
        milnet.eval()
        test_loss_bag, avg_score, aucs, thresholds_optimal = test(test_loader, milnet, criterion, args, save_path)
        if args.dataset=='TCGA-lung':
